...
void Deoptimization::reassign_fields(frame* fr, RegisterMap* reg_map, GrowableArray<ScopeValue*>* objects, bool realloc_failures, bool skip_internal, TRAPS) {
  for (int i = 0; i < objects->length(); i++) {
    ...
    if (EnableVectorSupport && (VectorSupport::is_vector(k) || VectorSupport::is_vector_payload_mf(k))) {
      assert(sv->field_size() <= 2, "%s not a vector", k->name()->as_C_string());
      ScopeValue* payload = sv->field_at(0);
      if (payload->is_location() &&
           (payload->as_LocationValue()->location().type() == Location::vector ||
            payload->as_LocationValue()->location().type() == Location::dbl    ||
            payload->as_LocationValue()->location().type() == Location::lng) ) {
#ifndef PRODUCT
        if (PrintDeoptimizationDetails) {
