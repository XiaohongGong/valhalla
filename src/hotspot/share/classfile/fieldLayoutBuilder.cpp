...
void FieldGroup::add_multifield(FieldInfo* field, InlineKlass* vk, Array<MultiFieldInfo>* multifield_info) {
  assert(field->is_multifield() || field->is_multifield_base(), "Must be");
  u2 base = field->multifield_base(multifield_info);
  BasicType type = Signature::basic_type(field->signature());
  ...
  if (mfg == NULL) {
    ...
    mfg->add_field(fs, vk, multifield_info);
    _multifields->append(mfg);
  } else {
    mfg->add_field(field, vk, multifield_info);
  }
}
...
void MultiFieldGroup::add_field(FieldInfo* field, InlineKlass* vk, Array<MultiFieldInfo>* multifield_info) {
  assert(field->is_multifield() || field->is_multifield_base(), "Must be");
  assert(field->multifield_base(multifield_info) == multifield_base(), "multifield base mismatch");
  jbyte idx = field->multifield_index(multifield_info);
  if (_fields == NULL) {
    _fields = new GrowableArray<LayoutRawBlock*>(INITIAL_LIST_SIZE);
  } else {
    guarantee(field->signature() == signature(), "multifield signature mismatch");
  }
  BasicType type = Signature::basic_type(field->signature());
  ...
  if (type == T_PRIMITIVE_OBJECT) {
    ...
    int size = type2aelembytes(type);
    block = new LayoutRawBlock(fs.index(), LayoutRawBlock::MULTIFIELD, size, size, false, field->multifield_index(multifield_info));
  }
  int cursor = 0;
...
void FieldLayoutBuilder::regular_field_sorting() {
  ...
  for (GrowableArrayIterator<FieldInfo> it = _field_info->begin(); it != _field_info->end(); ++it, ++idx) {
    ...
    if (fieldinfo.is_multifield() || fieldinfo.is_multifield_base()) {
      group->add_multifield(&fieldinfo, _multifield_info);
    } else {
      switch(type) {
      case T_BYTE:
      case T_CHAR:
